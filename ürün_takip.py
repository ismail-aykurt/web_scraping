...
# Merhaba arkadaşlar bu eğitimde begendiğğiniz bir bilgisayar var o modelin düşüp düşmediğini
#  takip  etmek için python ile kazıyaraak takip etmek istiyorum
...
def urun_kontrol():
    ...
    # Evet verileri başarılı bir şekilde aldık şimdi bana lazım olan ürün özelikleri ve fiyatı bunun için sayfanın
    # kaynak koduna gidip ona uygun kodu alıp bulacağız find_all kommutu ile

    bilgiler = soup.find(id="productTitle").text.strip()

    # şimdi de ürünün fiyatını alalım

    fiyat = soup.find("span", class_="a-price-whole").text.strip()
    fiyat = int(fiyat.replace(".", "").replace(",",
                                               ""))  # burda da fiyatta bulunan nokta ve virgül değerini kaldırdım daha güzel gözüksün diye
    # daha sonra int değişkenine çevirdim

    # şimdi de bu verileri kendime e post olarak albilirim mesela ben fiyat düşünce bana posta olarak gelsin isteyebilirim
